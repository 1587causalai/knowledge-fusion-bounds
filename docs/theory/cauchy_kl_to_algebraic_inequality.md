...
## 9. 反例分析：数值实验对代数不等式的挑战

我们最近的数值实验发现了一个参数组合，它不仅挑战了 $D_{KL}(P_0 \| P_{fused}) \le \sum D_{KL}(P_0 \| P_k)$ 的形式，也挑战了本文提出的 $D_{KL}(P_0 \| P_{fused}) \le 2 (D_{KL}(P_0 \| P_1) + D_{KL}(P_0 \| P_2))$ (即 $N=2, C=2$ 的情况)。

对应的代数不等式是：
$$ k(x_1+x_2, u_1 u_2) \le (k(x_1,u_1) \cdot k(x_2,u_2))^2 $$

考虑同向扰动，即 $x_1=x_2=x$ 且 $u_1=u_2=u$。不等式变为：
$$ k(2x, u^2) \le k(x,u)^4 $$

### 数值反例参数
从实验中，我们发现当 $\Delta\mu = 0.5$ 且 $\Delta s = -0.5$ 时出现反例。假设基准分布 $\gamma_0=1$。
则对应的无量纲参数为：
- $x = \Delta\mu / \gamma_0 = 0.5 / 1 = 0.5$
- $u = e^{\Delta s} = e^{-0.5} \approx 0.6065306597$

### 计算各项

**左边 (LHS): $k(2x, u^2)$**
- $2x = 2 \times 0.5 = 1.0$
- $u^2 = (e^{-0.5})^2 = e^{-1} \approx 0.3678794412$

$k(1.0, e^{-1}) = \frac{(1+e^{-1})^2 + (1.0)^2}{4e^{-1}} = \frac{(1+0.3678794412)^2 + 1}{4 \times 0.3678794412}$
$= \frac{(1.3678794412)^2 + 1}{1.4715177647} = \frac{2.8711130000}{1.4715177647} \approx 1.951114665$

**右边 (RHS): $k(x,u)^4$**
- $x = 0.5$
- $u = e^{-0.5} \approx 0.6065306597$

$k(0.5, e^{-0.5}) = \frac{(1+e^{-0.5})^2 + (0.5)^2}{4e^{-0.5}} = \frac{(1+0.6065306597)^2 + 0.25}{4 \times 0.6065306597}$
$= \frac{(1.6065306597)^2 + 0.25}{2.4261226388} = \frac{2.8309375129}{2.4261226388} \approx 1.166858025$

$k(x,u)^4 \approx (1.166858025)^4 \approx (1.361558476)^2 \approx 1.853852204$

### 比较
LHS $\approx 1.951114665$
RHS $\approx 1.853852204$

我们得到 $1.951114665 \not\le 1.853852204$。

**结论：**
对于参数 $x=0.5, u=e^{-0.5}$，代数不等式 $k(2x, u^2) \le k(x,u)^4$ 不成立。
这意味着原始的KL散度不等式 $D_{KL}(P_0 \| P_{fused}) \le 2 (D_{KL}(P_0 \| P_1) + D_{KL}(P_0 \| P_2))$ 对于这个特定的柯西分布参数组合也是不成立的。
